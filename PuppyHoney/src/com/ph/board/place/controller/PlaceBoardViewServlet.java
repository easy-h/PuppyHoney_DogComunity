...
public class PlaceBoardViewServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(plBoard!=null)
		{
			...
			List<PlaceBoardComment> pReplyList=new PlaceBoardService().selectPlaceBoardReplyList(plBoardNum);
			request.setAttribute("pReplyList", pReplyList);
			view="/views/board/board_place/placeBoardView.jsp";
		}else
		{
			request.setAttribute("msg", "조회한 게시물이 없습니다");
			request.setAttribute("loc", "/board/placeBoardList");
